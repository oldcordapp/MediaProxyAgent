...
class MediasoupSignalingDelegate {
  ...
  async start(public_ip, portMin, portMax, debug_logs) {
    ...
    console.log(
      `[MEDIA PROXY AGENT] Media Server online on ${this.ip}:${this.port}`,
    );
  }
  ...
  async onOffer(client_build, client_build_date, client, sdpOffer, codecs) {
    ...
    const legacyAnswer =
      client_build === "january_23_2017" ||
      client_build_date.getFullYear() < 2017;
    ...
    const rtpHeaders = Array.from(offer.medias[0].extensions.entries()).map(
      ([id, uri]) => {
        return { uri, id };
      },
    );
    ...
    const transport = await room.router.router.createWebRtcTransport({
      listenInfos: [
        { ip: "0.0.0.0", announcedAddress: this.ip, protocol: "udp" },
      ],
      enableUdp: true,
      enableTcp: true,
      preferUdp: true,
