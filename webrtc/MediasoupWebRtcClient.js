...
class MediasoupWebRtcClient {
  ...
  async onJoinedRoom() {
    ...
    await Promise.all(
      Array.from(clients).map(async (client) => {
        ...
        if (audioConsumer) {
          consumerAudioSsrc =
            audioConsumer.rtpParameters?.encodings?.[0]?.ssrc ?? 0;
        }

        if (videoConsumer) {
          consumerVideoSsrc =
            videoConsumer.rtpParameters?.encodings?.[0]?.ssrc ?? 0;
          consumerRtxSsrc =
            videoConsumer.rtpParameters?.encodings?.[0]?.rtx?.ssrc ?? 0;
        }
  ...
  async subscribeToTrack(user_id, type) {
    ...
    const existingConsumer = this.consumers?.find(
